...
public class MarkMapper {
    ...
    public int merge(Mark d) {
        SqlBuilder param = SqlBuilder.build().append("REPLACE INTO trade (" + COLUMNS + ") VALUES ")
    ...
    public List<Mark> list() {
        SqlBuilder param = SqlBuilder.build().append("select t2.code, t2.market, t2.date, t2.price, t2.rate_buy, " +
    ...
    public List<Mark> list(String code, String market) {
        SqlBuilder param = SqlBuilder.build().append("select " + COLUMNS + " from trade " +
                "where code = ? and market = ? order by date asc", code, market);
        return baseDB.list(param, mapper);
    }

    public int delete(String code, String market, String date) {
        SqlBuilder param = SqlBuilder.build().append("delete from trade  where code = ? and market = ? and date = ?",
                code, market, date);
        return baseDB.executeUpdate(param);
    }

    public Mark find(String code, String market, String date) {
        SqlBuilder param = SqlBuilder.build().append("select " + COLUMNS + " from trade  where " +
