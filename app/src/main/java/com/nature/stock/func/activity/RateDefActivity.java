...
import com.nature.stock.common.constant.DefType;
import com.nature.stock.common.manager.DefinitionManager;
...
public class RateDefActivity extends AppCompatActivity {
    ...
    private final ListDefManager listDefManager = InstanceHolder.get(ListDefManager.class);
    ...
    private final List<ExcelView.D<RateDef>> ds = Arrays.asList(
            ...
            new ExcelView.D<>("类型", d -> TextUtil.text(listDefManager.getTypeName(d.getType())), C, E, Sorter.nullsLast(RateDef::getType)),
    ...
    private void popEditDef() {
        PopUtil.confirm(context, "添加显示规则", editShow(), () -> {
            ...
            String dateEnd = null;
            Integer count = null;
            if (ListDefManager.DEFINED.equals(type)) {
    ...
    private void refreshType() {
        type.refreshData(listDefManager.listAllType());
    }
    ...
    private LinearLayout editShow() {
        ...
        type.mapper(listDefManager::getTypeName);
        type.init();
        this.refreshType();
        line.addView(n1);
        line.addView(n2);
