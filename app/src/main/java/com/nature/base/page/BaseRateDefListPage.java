...
public abstract class BaseRateDefListPage extends ListPage<RateDef> {
    ...
    protected List<RateDef> listData() {
        List<RateDef> list = this.manager().list(this.typeCode);
    ...
    private void doAdd() {
        this.doEdit((c) -> this.manager().find(this.typeCode, c) != null, RateDef::new);
    }
    ...
    private Consumer<RateDef> delete() {
        return d -> PopUtil.confirm(context, "删除-" + d.getTitle(), "确认删除吗？", () -> {
            this.manager().delete(this.typeCode, d.getCode());
            this.refreshData();
    ...
    protected abstract BaseRateDefManager manager();
